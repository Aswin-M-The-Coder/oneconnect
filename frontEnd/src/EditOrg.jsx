...
function EditOrg() {
	...
	useEffect(()=> {
		axios.get('http://localhost:8081/getorgdata/'+id)
		.then(res => {
			setData({...data, name: res.data.Result[0].name,
				email: res.data.Result[0].email,
				address: res.data.Result[0].address,
				donation: res.data.Result[0].donation
			})
		})
	...
	const handleSubmit = (event) => {
		...
		axios.put('http://localhost:8081/updateorgdata/'+id, data)
		.then(res => {
  ...
  return (
    <div className='d-flex flex-column align-items-center pt-4'>
			...
			<div class="col-12">
				...
				<div class="col-12">
					<label for="inputSalary" class="form-label">Total Donation</label>
					<input type="text" class="form-control" id="inputSalary" placeholder="Enter Salary" autoComplete='off'
					onChange={e => setData({...data, salary: e.target.value})} value={data.donation}/>
				</div>
